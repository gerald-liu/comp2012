...
Folder::Folder(unsigned int size): maxSize{ static_cast<int>(size) }, currSize{ 0 } {
    files = new const File* [currSize];
}

Folder::~Folder() { delete[] files;	}

Folder::Folder(const Folder &folder): maxSize{ static_cast<int>(folder.maxSize) }, currSize{ folder.currSize } {
    files = new const File* [currSize];
...
void Folder::print() const { for (int i=0; i<currSize; i++) files[i]->print(); }
